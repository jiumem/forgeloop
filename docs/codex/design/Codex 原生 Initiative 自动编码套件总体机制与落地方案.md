...
设计规划循环在本文中先只保留法位，不展开具体机制。

当前只封板三件事：

- 它负责对象划分、Milestone 切法、Task 拆解与参考入口分配
- 它的输出必须是可进入编码执行循环的合法总任务文档
- 当编码执行循环暴露边界失真、阶段失真或计划失真时，问题必须回流到这个循环，而不是在执行层硬修
...
- 做 preflight、状态重建、frontier 选择和 ready task 选择
- 决定当前进入哪个子循环
...
第二，**当前版本先写实编码执行循环，设计规划循环只保留法位占位。**

第三，**这套系统是 Initiative-first，不是 task-first。**

第四，**Milestone 是正式阶段边界，Task 是内部最小原子。**

第五，**Task 内可以吸收 delegate mechanics，但不能让 delegate framework 升格为总框架。**

第六，**Codex 原生运行主形态应明确为 Supervisor + bounded subagents。**

第七，**Agent 间主通信协议应以 `Global State Doc + 对象级滚动通信文档` 为主，而不是把代码接口调用当成一级协议。**

第八，**Supervisor 主 Agent 必须保留正式调度与汇总责任，不能空壳化。**

第九，**Formal Gate / Review 仍只认 Task / Milestone / Initiative 三层正式收口；Shadow 只保留早发现，不新增 Shadow Review 法位。**

第十，**用户体验必须被压缩为：启动一个 Initiative，让系统持续推进，只在正式断点介入。**

...
- `script`：确定性执行核，承载 parser、state rebuild、packet builder、gate runner 等可测试逻辑
...
- 必须精确到 packet / object key / artifact ref
...
- 说明会读写 `.initiative-runtime/` 的哪些对象
...
- 说明是否允许调 `task_worker`、`spec_reviewer` 等
...
- `Run the task-local loop for a ready Task: build packet, dispatch implementer, run spec check, run quality check, and stop at READY_FOR_ANCHOR or escalation.`
...
- 调 implementer 类 agent 时，必须给出经过裁剪的 packet
- 调 reviewer 类 agent 时，必须给出 check / review packet
...
### 8.4 与 runtime cache 的接缝

skill 可以读写 `.initiative-runtime/`，但必须遵守：
...
- 显式传入 `task_packet`、`spec_check_packet`、`review_packet`

...
第八，reviewer 热路径必须自足。
fresh reviewer 进入时，默认热路径必须能同时看到：

- 当前对象是谁
- 当前 round 的 `review_handoff`
- 当前正在回应哪条上一轮 `review_result`

因此，`current-effective` 应作为 fresh reviewer 的默认热路径入口；
`round-scoped` 只是局部 round inspection helper，不能替代 fresh reviewer 的最小自足入口。
...
- 持续作为同一个执行 owner，把当前 candidate、证据和正式 reviewer handoff 追加到当前层级的 review rolling doc

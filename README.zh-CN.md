...
[快速开始](README.md) · [3.3 发布说明](docs/releases/3.3.0-release-notes.md) · [3.2 → 3.3 迁移指南](docs/migrations/3.2.0-to-3.3.0.md)
...
- `$code-review`：评审范围明确的已实现代码，可使用 PR、Diff、工作区、文件、目录或模块作为范围。
- `$diagnosing-bugs`：定位复杂 Bug、失败或性能退化的根因。
- `$triage`：整理一组待办、故障或反馈并决定下一步。
- `$handoff`：把当前工作压缩成另一个任务可以继续的交接包。

其中 `code-review` 和 `diagnosing-bugs` 也允许在其他 Workflow 中按需复用，因此不是仅用户调用入口。
...
| Skill | 合适的触发场景 |
| --- | --- |
| `code-review` | 从预期行为与仓库规范两轴评审范围明确的已实现代码 |
...
| 上游 `code-review` | 3.0–3.2 的 `review-change`；3.3 起统一为 `code-review` |

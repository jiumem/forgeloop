...
Write every complete Ticket body, the `Ticket → STANDARD | HIGH_RISK → evidence` classification, and the complete `Invariant → Owning Ticket` mapping to one complete draft document before asking for approval. Identify the exact draft version with a readable reference. In the conversation, show that reference and summarize each Ticket. Do not repeat the complete Ticket bodies in the conversation. When the parent declares `None`, show `Invariant ownership: None`. After any revision, update the same draft document, identify its new exact draft version, and show only the changed Tickets and fields before requesting reapproval. Then summarize each Ticket:
...
Only after approval, create one initial fresh, isolated, read-only Design Reviewer for each approved `HIGH_RISK` Ticket; skip Design Reviewers for `STANDARD` Tickets. Bind each Reviewer to the parent Spec revision, relevant ADRs, complete Ticket draft, blocking edges, code evidence, and referenced invariants. It must not modify files, the Spec, ADRs, draft Tickets, or Tracker state, and must not propose unapproved product behavior. Ask it to use counterexamples to check that the model is finite, facts have one authoritative source, failure and recovery close the loop, no hidden caller constraints remain, and Proof is credible.

A Design Reviewer is a contract-bound falsifier, not an acceptance authority or solution designer. Classify each concern as `BLOCKING_GAP | CONTRACT_QUESTION | HARDENING_RECOMMENDATION | REVIEW_BLOCKED`. Every `BLOCKING_GAP` must contain `authority_ref`, `observable_violation`, `reachable_counterexample`, `necessity`, and `required_proof`. Its `authority_ref` must identify an exact approved Delivery Acceptance, Cross-seam Invariant, applicable ADR, or approved failure behavior; its counterexample must be reachable inside that approved failure model; and its necessity must explain why the smaller complete design cannot satisfy that authority. A concern without that complete binding cannot be a `BLOCKING_GAP`. Every `CONTRACT_QUESTION` must contain `authority_ref`, `observable_violation`, `reachable_counterexample`, `necessity`, `required_proof`, and `missing_decision`. Its `authority_ref` and counterexample follow the same approved-authority boundary as a `BLOCKING_GAP`, and its `necessity` must show why no Ticket-local choice can satisfy that authority without inventing product or architecture behavior. An unbound `CONTRACT_QUESTION` cannot block publication; the owning Agent must `REJECT` it for missing authority, an unreachable counterexample, or Scope expansion, or classify optional resilience or infrastructure work as `HARDENING_RECOMMENDATION`. A completely bound `CONTRACT_QUESTION` stops publication through `CONTRACT_BLOCKER` and returns the decision to its owning workflow. Only a `HARDENING_RECOMMENDATION` cannot block publication. Do not strengthen durability, confidentiality, availability, cleanup, recovery, or other guarantees beyond their approved boundary. Distinguish transient implementation residue from durable product facts. Any concrete mechanism from the Reviewer is only a non-binding implementation suggestion and never becomes an acceptance requirement by being mentioned.

Before changing a draft, the owning Agent must adjudicate every concern as `ACCEPT | NARROW | DEFER | REJECT | CONTRACT_BLOCKER`. `ACCEPT` admits a contract-bound gap; `NARROW` admits the observable problem but selects the smallest complete remedy; `DEFER` records non-blocking hardening; `REJECT` records missing authority, an unreachable counterexample, or Scope expansion; and `CONTRACT_BLOCKER` returns a missing approved decision to its owning workflow without hiding it in Ticket design. Do not mechanically implement Reviewer suggestions. PASS means there is no unresolved admitted `BLOCKING_GAP`; rejected or deferred concerns and hardening recommendations may coexist with PASS. A PASS binds only its exact fixed inputs. If that Ticket body, blocking edges, Spec revision, or referenced ADR changes, invalidate the old Verdict, not the Reviewer identity, and continue the same Design Reviewer thread with the complete new fixed inputs, prior Findings, and their dispositions. Create a replacement Reviewer only when the original is unavailable, has lost read-only independence, or the review authority is replaced.

All admitted high-risk `BLOCKING_GAP` concerns must be resolved and receive a bound PASS before publication. A `CONTRACT_QUESTION` that exposes a missing approved decision returns `CONTRACT_BLOCKER` without changing the contract only when its complete binding passes the requirements above; tell the user to invoke the owning workflow explicitly. `CONTRACT_BLOCKER` may follow only a completely bound `CONTRACT_QUESTION`, never the Reviewer's unsupported assertion that a decision is missing. A bound `CONTRACT_QUESTION` also prevents PASS until its owning workflow resolves it; rejected unbound questions do not. For `REVIEW_BLOCKED`, report the unreadable or invalid fixed input and do not treat missing evidence as PASS. Do not publish `STANDARD` Tickets early while an admitted blocking concern remains. These classifications, dispositions, and Verdicts are Agent-readable design judgment and evidence, not Tracker fields, a parser, a state machine, or a workflow DSL.

After a third consecutive non-PASS review round for the same Ticket, stop the ordinary revision loop and do not start a fourth ordinary review round. Perform a scale review before any further Design Review: compare the current draft with the initially approved draft; map every newly introduced fact source, owner, lifecycle, state model, storage mechanism, and failure guarantee to its exact approved authority; and remove or defer every unsupported mechanism. Decide whether the remaining problem is a real contract gap, an unstable draft, or Reviewer Scope expansion. A previously rejected or deferred concern cannot block again without new contract authority or materially new evidence. Resume review only for the smallest resulting draft, or return `CONTRACT_BLOCKER` to revise the parent contract in place. This is a semantic stop condition, not a parser-driven round state machine.

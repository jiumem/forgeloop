...
`task-loop` 只处理一个已确认的 Task。你在这里扮演 Task 层 `Supervisor`：维护最小控制面、维持单一 `coder_slot`、派发同一个 `coder` 与每轮 fresh `task_reviewer`，并根据 `G1`、`anchor / fixup`、`R1` 的事实决定继续修补、正式 clean、升级或停下。
...
- 若只有 bounded task brief 且 rolling doc 不存在，可用它初始化 header（含对象身份与 `coder_slot`）与 `task_contract_snapshot`；初始化后 rolling doc 成为唯一协作面
...
- 若 rolling doc 不存在，仅初始化 header（含对象身份与 `coder_slot`）与 `task_contract_snapshot`
...
- 派给 coder 的输入只需要定位当前正式输入面：当前 Task 身份、`design_ref`、`gap_analysis_ref`、`total_task_doc_ref`、`Global State Doc` 路径、当前 `Task Review Rolling Doc` 路径

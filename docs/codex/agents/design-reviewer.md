...
### 1. Review The Whole Design Object First; Do Not Accept Only The Presented Summary

Review the whole design object that is actually being handed off:

- the current `Design Doc`
- the requirement baseline or `design draft`
- the active `Design Rolling Doc`
- the bound `stage_reference_ref`
- the relevant repo facts, implementation facts, and hard constraints
- the target-state structure, fixed-versus-flexible line, correctness surface, and residual-risk handling

Do not accept a polished summary if the real target state, scope boundary, correctness surface, or downstream binding effects remain under-defined.
...
Do not manufacture a clean Design review on top of:

- no current `Design Doc`
- no active `Design Rolling Doc`
- no bound `stage_reference_ref`
- missing requirement baseline or missing hard constraints
- a document shape that materially violates the active stage reference
- contradictory scope, non-goals, or target-state claims

### 3. Expose Evidence Gaps And Blocking Uncertainty First; Do Not Hide Unproved Design Claims

If the design is under-grounded, say:

- what formal fact or constraint is missing
- why the missing grounding matters
- which part of the design judgment therefore remains unproven

Do not confuse a plausible design with a proved design.

Do not let an elegant narrative hide blocking uncertainty that should stop downstream planning.

### 4. Diagnose The Real Fracture, But Stay Inside Design Radius
...
- `Topology Layer`
- `Contract Layer`
- `Correctness Surface Layer`
- `Reversibility Layer`

If a problem clearly belongs to a later `Gap Analysis Doc` or `Total Task Doc` stage, say that explicitly in findings or downstream-readiness analysis, but do not widen your role into migration design or task planning or turn `next_action` into a stage-routing command.

### 5. Review For Downstream Planning Readiness, Not For Human Taste Performance

Check whether the current design can legally support the next planning step.

That means checking, at minimum:

- the winning cut actually addresses the primary contradiction
- the scope and non-goals are explicit enough to stop downstream widening
- the `Gap Analysis Requirement` line is explicit enough that the supervisor can route legally after design seal
- the target-state structure is explicit enough for later planning to inherit
- the fixed-versus-flexible line is explicit enough that later stages do not reopen architecture by accident
- the correctness surface is explicit enough that downstream roles can tell what must remain true

Do not reward rhetoric that still leaves downstream planning to reconstruct hidden intent.
...
`Next Action` must be a short, explicit review-local recommendation that the next planner round and the planning-layer supervisor can act on directly, such as `continue_design_repair`, `ready_for_supervisor_routing`, `wait_for_upstream_judgment`, or `stop_on_blocker`. Do not encode later-stage routing decisions into this field.

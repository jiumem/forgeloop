...
- The Scheduler must refresh the Ticket Coder and both Ticket Reviewers only when entering Cycle 2: continue the fresh Correction Coder that performed Exhaustion Diagnosis, create two fresh isolated Reviewers, and reuse them throughout Cycle 2. Do not refresh them between its repair rounds and do not create a third-cycle set.
...
- After Scheduler-task recovery, reconstruct the role from durable Tracker and Git facts and reuse a still-valid live child when possible. Create a fresh child only when the existing child is unavailable, has lost its required context or read-only independence, or its contract authority materially changed. Child availability is never required for recovery.
...
- For Final Acceptance, verify only Seal Eligibility from existing Ticket Verdicts, Integration Results, Final Gate evidence, effective revisions, membership, and final target bindings. The Scheduler does not judge product correctness, create a final Reviewer, run a new semantic validation, or create an implementation Finding. The Scheduler must refresh the target again before rendering an Acceptance `PASS` Payload; this refresh occurs immediately before rendering.

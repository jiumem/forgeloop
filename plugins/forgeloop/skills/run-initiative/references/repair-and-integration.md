...
- `NO_REPAIR`: legal only when all remaining Blocking Findings are Spec-axis Findings and exact evidence breaks their necessity chain: they lack approved authority or a necessary consequence, depend on a counterexample outside the approved operating and failure model, identify no observable Candidate failure or non-credible proof, demand only a stronger undeclared guarantee, or the Candidate already satisfies the required result. Make no Candidate change and consume no repair round. The Scheduler routes one evidence-only reconsideration to the same Spec Reviewer under [scheduler.md](scheduler.md); a Standards-axis Finding is never routed through this path.
...
When either axis returns `REPAIR_REQUIRED`, wait for the other axis, then continue the original Coder with both complete Finding sets. Do not alter Reviewer evidence or merge the two axes. The Coder independently verifies authority and the complete necessity chain before diagnosing a mechanism, then selects the smallest complete repair by semantic disturbance; the Scheduler only validates and routes the declared diagnosis. Continue each original Reviewer with the new fixed Head and only that Reviewer's own axis history.
...
- `AUTO_REPAIR_RENEWAL`: legal only after Cycle 1, when the Agent concludes that at least one authority-and-necessity-bound violation remains and a materially different, credible, falsifiable correction stays in Scope and converges or reduces the design. Compete to publish and exactly confirm `RUN_RESUMED` with reason=`AUTO_REPAIR_RENEWAL` without user approval; only the winning native Resume may authorize Cycle 2 Candidate mutation.

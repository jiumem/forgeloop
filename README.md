...
```text
澄清 Requirement -> 推荐或选择 Initiative -> 写 PLAN -> 按 Milestone 执行 -> Coder 交付 -> Reviewer 审查 -> PASS 后继续
```

其中 Initiative 是用户入口和顶层专项对象，可以粗略类比传统项目管理中的 Epic，但在 Forgeloop 中不是需求聚合标签，而是必须能被 `PLAN.md` 承载、被 Milestone 分段交付、被 Reviewer 审查并最终归档的工程闭环。它可以是一个新功能、一组架构改造、一次测试体系加固、一轮性能优化、一段 API/Schema 治理、一次文档和示例完善、一个迁移工程，或者任何需要拆成多个阶段持续推进的开发目标。
...
- 用 `grill-requirement` 把模糊需求或重构想法审问到可规划状态；
...
Initiative 是用户入口，也是 Forgeloop 的顶层开发单元。它可以帮助熟悉传统项目管理的人粗略理解为 Epic 级工作，但在 Forgeloop 里拥有更强的执行法位：必须能被写成 `PLAN.md`，必须能拆成多个 Milestone，必须有可验证的完成标准，并且最终必须通过交付归档收口。
...
在模糊需求、重构想法、迁移计划或架构方向进入 PLAN 前，先把它当作设计决策树逐枝追问，直到足以判断是否适合进入 `write-plan`。

它不会写代码、issue、`PLAN.md` 或 `LEDGER.md`。它只输出裁决摘要：provisional requirement、disposition、confirmed decisions、open blockers、non-goals 和 recommended next step。

常见用法：

```text
请使用 Forgeloop grill-requirement 审问这个重构想法：我想把认证模块重构掉。
```
...
把用户选中的 Initiative 或已经澄清到可规划状态的需求写成可执行的 `PLAN.md`。如果需求仍有多个阻塞决策分支，应先使用 `grill-requirement`。
...
```text
请使用 Forgeloop grill-requirement 审问这个想法：我想把认证模块重构掉，但不确定范围。
```

适合在需求还没有边界、重构目标可能过大、或者你希望 Agent 在写 PLAN 前先展开决策树时使用。
...
- 极小 bug 修复；
- 不需要计划、不需要审查的临时实验；
- 需求还没有边界，且用户也不希望先用 `grill-requirement` 做前置澄清的问题；

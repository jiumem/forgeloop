...
> 当前版本：`3.5.0` · 20 个正式 Skill · 11 个用户入口 · 9 个模型可调用能力

[快速开始](README.md) · [3.5.0 发布说明](docs/releases/3.5.0-release-notes.md) · [3.3.0 → 3.3.1 迁移指南](docs/migrations/3.3.0-to-3.3.1.md)
...
`to-spec` 负责结构化已经解决的上下文，不承担漫长访谈。正式发布前，它会结合代码事实审计候选方案是否确实需要新增概念、事实源、生命周期和结构复杂度；能够在既有授权内收敛时形成更小的完整 Spec，需要新的产品或架构决定时返回上下文不足。`to-tickets` 不重新争论已批准设计，只把 Spec 拆成最小、可观察的 Ticket 图。
...
每个修复周期最多三轮普通修复。评审指出的实现问题回到本周期 Coder，因为它保留本周期的问题与评审历史；修复后由本周期的两名 Reviewer 对新固定 Candidate 重新双审。

候选提交自身导致的测试或契约失败也回到本周期 Coder；外部权限、远端服务或环境故障则暂停，不能伪装成代码缺陷消耗修复轮次。

第三轮后仍存在 Blocking Finding 时，Scheduler 先持久化并精确回读 `RUN_PAUSED / REPAIR_BUDGET`，再创建 fresh Coder 执行只读 Exhaustion Diagnosis。若旧机制已被证据证伪、存在实质不同且可证伪的 Scope 内新机制，并且刚结束周期产生真实净进展，则确认 `RUN_RESUMED / AUTO_REPAIR_RENEWAL`，在同一 Ticket、Run 和 Branch 上开启新的三轮周期，并使用 fresh Coder 与 fresh 双 Reviewer。该过程不需要用户批准，也不创建 replacement Ticket。

自动修复没有任意总周期上限，但每次续配都必须重新通过语义门禁。没有可信新机制或真实净进展时保持 `IMPLEMENTATION_BLOCKED`；无条件“继续”不能恢复。正确实现需要改变 Spec、Scope、Acceptance、ADR 或已批准接口时进入零预算 `CONTRACT_BLOCKER`。

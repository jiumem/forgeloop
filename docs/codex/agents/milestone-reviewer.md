...
- the appended `r2_result` block must at minimum include `kind`, `round`, `author_role`, `created_at`, `verdict`, `next_action`, and `required_follow_ups`; `author_role` must stay `reviewer`
...
`Next Action` must be a short, explicit next-step statement that the next coder and supervisor can act on directly, such as “continue current milestone repair”, “objectize a repair task and fall back to Task”, “enter the higher-level review entry”, or “wait for user input”.

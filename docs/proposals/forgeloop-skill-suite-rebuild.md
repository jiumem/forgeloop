...
Matt Pocock 上游已经定义的 Skill 行为、模板和流程直接从固定 Commit 继承，不重复设计。落地时只做 Forgeloop 品牌、Skill 名称、路径、跨 Skill 引用和 Codex 元数据等必要机械替换。全部正式 Skill 内容、References、Tracker 模板与 UI 元数据统一使用英文；项目 Proposal、Plan、Evidence 与迁移文档继续使用中文。Frontmatter `description` 是 Codex 触发元数据，不继承上游流程摘要：20 条统一由声明式清单生成，以 `Load when` 开头，只描述适用请求和相邻边界。上游的 Claude 专用 `disable-model-invocation` 不进入 `SKILL.md`；Codex 的调用策略统一写入 `agents/openai.yaml`。
...
Forgeloop 只为上游没有覆盖的自动执行、结构化 Verdict、修复预算、状态恢复、完成门禁、角色委派和多 Spec 验收补充协议。
...
Ticket 必须是窄而完整的垂直切片，能够在一个新鲜 Agent 上下文中完成，并通过公共 Seam 独立验证。每张 Ticket 拥有独立 Coder、Spec Reviewer 和 Standards Reviewer；同一 Ticket 的修复轮次复用这三个上下文，Ticket 完成后不再向其派发任务。
...
- 综合代码、测试、CI、Tracker、ADR 和用户已声明目标，给出 1–3 个跨类别候选；
...
- `run-initiative <spec-ref...>`：多 Spec 时先预览，按派生 `initiative_revision` 查询复用或幂等创建持久化 Initiative Tracker Item；写入结果不确定时先查询，不直接重试。
...
- Claim 失败者不得开始编码；
- `PAUSED` 保留 Claim 给原 Run；`RUN_CANCELLED` 停止派发、尽力中断当前 child，并只释放当前 Run ID 实际拥有的 Claim；
- Ticket 成功关闭后释放其原生 Claim；根 Acceptance PASS、根 Item 关闭后释放根 Scheduler Claim。Local 只允许锁中 Run ID 的所有者删除对应 lock；
- 不使用会误判长任务死亡的短 TTL；
...
`NO_CHANGE_REQUIRED` 返回 `Base == Head`、空 Commit 列表和现有行为证据，是唯一合法的空 Diff，仍需双 Reviewer 验证；`CONTRACT_BLOCKER` 持久化为 `RUN_PAUSED` reason=`CONTRACT_BLOCKER` 且不进入普通修复预算；`IMPLEMENTATION_BLOCKED` 不创建 Reviewer。
...
- 候选代码导致 Required Check 失败时返回原 Coder，并占用同一两轮普通修复预算；权限、基础设施或无关失败进入 `CHECKS_BLOCKED`；
...
Scheduler 使用通用 Codex child thread，并通过自包含的 Role Task Pack 定义 Coder、Spec Reviewer、Standards Reviewer 与 Acceptance Reviewer。首次委派不继承 Scheduler 对话；同一 Ticket 的修复继续原三条 child thread，跨 Scheduler 任务恢复则只从 Tracker 与 Git 持久事实创建 fresh child。

Role Task Pack 只提供角色、目标、角色协议路径、冻结引用与 Git 固定点、写入或只读 Scope、必需证据和停止条件，不承载宿主级 child 配置。Ticket 完成后不再派发到旧 thread；协议不依赖宿主提供物理关闭能力。
...
8. 单 Spec Run 最后关闭该 Spec；多 Spec Run 保持成员 Specs Open，等待 Initiative Gate。

每个 Spec 在集成后始终创建 fresh、隔离、只读的 Spec Acceptance Reviewer，不复用 Ticket Reviewer。多 Spec Initiative 必须先集成全部 Tickets，再冻结同一个最终目标 Commit，依次完成所有 Spec Acceptance，并保持成员 Specs Open；Initiative Acceptance PASS 后先关闭成员 Specs，最后关闭 Initiative。`REVIEW_BLOCKED` 与 `ACCEPTANCE_BLOCKED` 只修复 Scheduler 输入或暂停，不进入普通 Repair。恢复对相同冻结输入的读取时只继续 blocked 轴；共享输入内容改变时两轴都重新审查。

Acceptance `REPAIR_REQUIRED` 时所有相关父项保持打开，并按 Acceptance 层级、Parent、Revision、最终 Commit 与 `finding_id` 为每个 Finding 派生 `repair_key`。`$to-tickets` 的 Forgeloop Acceptance Repair Mode 逐 key 查询并复用 Open Ticket；多个 key 只有在同一原子垂直切片中才能指向同一 Ticket。Spec Finding 归属该 Spec；Initiative Finding 必须路由到 Scope 能覆盖它的现有成员 Spec，不创建 Initiative 直属 Ticket，无法合法归属时转 `CONTRACT_BLOCKER`。Repair 改变最终 Commit 后重新执行所有 Spec Acceptance，不引入 reopen 状态。
...
5. 所有交付代码进入最终目标分支；
6. fresh Initiative Acceptance Reviewer 返回 `ACCEPTANCE_RESULT level=INITIATIVE verdict=PASS`；
...
- 实质性修改必须由用户确认，记录 `RUN_PAUSED` reason=`SPEC_CHANGE`，Initiative 对外进入 `PAUSED_FOR_SPEC_CHANGE`；
- 用户显式调用 `$to-tickets` 重新对账 Open Tickets，`run-initiative` 不得自行或隐式启动该 Workflow；已完成 Tickets 保留历史；
- `$to-tickets` 只对受影响 Open Tickets 提出 `retain`、`update`、`supersede`、`create` 动作并等待用户批准，不自动重拆完整 Spec；
...
- Ticket Coder、双 Reviewer、Role Task Pack、修复预算和 fresh 最终验收协议；

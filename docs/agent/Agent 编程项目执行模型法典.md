...
> **对应地，验证体系分为 G1 / G2 / G3 三层阻断门，另设预警校验（内部正式名为 Shadow Check Profile）作为非阻断预警层；审查体系分为 R1 / R2 / R3 三层正式审查，另设预警审查（内部正式名为 Shadow Review Profile）作为非阻断预警层。**
...
| SR      | **Shadow Review Profile**     | 预警审查    | non-blocking |

特别强调：

> **Task 内部的规范符合性检查与代码质量检查，不构成正式 R1。**
...
### 9.1 第一原则：每一层对象必须拥有唯一可指派的法定参考入口
...
### 9.3 Milestone：Milestone Reference Doc
...
### 9.4 Initiative：Initiative Reference Doc
...
探索性 `wip` 可以存在；局部辅助执行与辅助检查也可以存在，但它们都不改变 Task 的正式法位。
在沉淀 `anchor commit` 之前，允许先做 **规范符合性检查** 与 **代码质量检查** 这两类局部内部检查，以帮助当前 Task 收口。
但这些内部检查只服务于局部实现，不构成正式 R1。
当局部收口完成后，仍必须沉淀出一个 `anchor commit`。
此时跑 G1，通过后才允许进入 R1。

于是 Task 的默认节奏是：

> **实现 → 规范符合性检查 → 代码质量检查 → anchor commit → G1 → R1 → 继续推进**
...
开发者 / Agent 可以持续 push。
push 后触发预警校验与必要时的预警审查，用于尽早发现问题，但不阻断主流程。

于是 push 的节奏是：

> **push → 预警校验 → 预警审查（必要时）→ 预警反馈**
...
| 非正式层                  | 动作                   | 审查               | 性质     |
| --------------------- | -------------------- | ---------------- | ------ |
| **Branch Checkpoint** | push / periodic scan | 预警审查           | 预警，不放行 |

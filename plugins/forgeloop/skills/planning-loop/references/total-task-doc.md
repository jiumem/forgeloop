...
- Initiative criterion、Milestone、Task 与 acceptance index 这类对象级条目，必须继续使用 `<parent>/<child>/<object-key>` 形式的 object-local 锚点。
...
<!-- forgeloop:anchor document-card -->
- 顶部 `状态` 是文档自身的正式状态标记。`round`、`handoff` 与 `review history` 仍以控制状态文档和 rolling doc 为准；若两者不一致，`Supervisor` 必须先修正文档状态，不能让下游自行推断。

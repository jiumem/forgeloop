...
套件自带的 custom agent manifest 位于 [`agents/`](agents)。当前覆盖 runtime workflow 主角色 `coder`、`task_reviewer`、`milestone_reviewer`、`initiative_reviewer`，以及 `design_challenger`、`plan_reviewer`、`code_reviewer` 等辅助角色；安装脚本会把它们落到目标项目的 `.codex/agents/`。

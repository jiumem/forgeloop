...
4. `flat-tasks-loop` 通过串行派发 `task-loop` 来执行整份计划；`task-loop` 也可以直接执行单个有边界的任务。
...
套件自带的 custom agent manifest 位于 [`agents/`](agents)。当前包含 `design_challenger`、`plan_reviewer`、`implementer`、`spec_reviewer`、`code_reviewer` 等边界明确的角色；安装脚本会把它们落到目标项目的 `.codex/agents/`。

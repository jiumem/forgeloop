...
8. On any repair trigger, continue the current-cycle Coder for an independent read-only Repair Diagnosis turn with the complete evidence and history. Validate its fields and route `NO_REPAIR`, `LOCAL_REPAIR`, `STRUCTURAL_REPAIR`, or `CONTRACT_BLOCKER` without judging technical merits. `NO_REPAIR` is legal only for Spec-axis Findings, receives one evidence-only reconsideration from the same Spec Reviewer on the unchanged Head, and consumes no round; the Standards Reviewer does not participate. Only an authorized `LOCAL_REPAIR` or `STRUCTURAL_REPAIR` may enter a code-changing turn. Effective code or test tree changes invalidate both Verdicts and consume one of at most three rounds in that Cycle; metadata-only and evidence-only changes do not. After Cycle 1 exhausts, confirm `RUN_PAUSED / REPAIR_BUDGET` and create a fresh Correction Coder for read-only Exhaustion Diagnosis. Only when it returns a credible `AUTO_REPAIR_RENEWAL` and the Resume wins exact native read-back may the Scheduler refresh both Ticket Reviewers and enter Cycle 2 without user approval; the Correction Coder becomes the Cycle 2 Coder. Otherwise persist `IMPLEMENTATION_BLOCKED` or `CONTRACT_BLOCKER` and stop automatic mutation. After Cycle 2 exhausts, stop with either of those outcomes and never create Cycle 3.
...
12. For a single-Spec Run, after all ordinary Tickets complete, let the Spec Root Final Integration Gate deliver a `SHARED` Spec to the target; `INDEPENDENT` uses its existing Ticket Integration Results. Then let the Scheduler evaluate Spec Seal Eligibility on the actual final Commit, publish `ACCEPTANCE_RESULT level=SPEC result=PASS`, and close the Spec only after exact native read-back.
13. For a multi-Spec Run, after all Initiative Tickets complete, execute each SHARED member Spec's Final Integration Gate independently. After every member delivery enters the target, freeze one final Commit and let the Scheduler evaluate each member Spec Seal Eligibility sequentially against that same Commit while keeping member Specs Open.
14. After all provisional member Spec results bind that Commit, let the Scheduler evaluate Initiative Seal Eligibility. Only after the Initiative `PASS` forms the root Acceptance Seal may it close the member Specs and then the Initiative parent last. After root closure, release the root Scheduler Claim according to the configured runtime.

## Delivery Completion Report

Final Acceptance seals only existing formal Spec evidence and delivery facts. If a completed Spec declares a `Release Boundary`, close it under the existing single- or multi-Spec rule and report: `Delivery is complete; Release was not executed by this Run`, followed by its Post-delivery action and Tracking reference.
